...
for i in range(34):
    file = "./redlight/" + "red{0:02d}.png".format(i+1)
    img = cv2.imread(file)
    if img is None:
        print("파일이 없습니다")
        break
...
plot_images(n_row=2, n_col=17, images=Redlight_images)

# GREEN Light list
Greenlight_images = list()
for i in range(28):
    file = "./greenlight/" + "green{0:02d}.png".format(i+1)
    img = cv2.imread(file)
    if img is None:
        print("파일이 없습니다")
        break
...
plot_images(n_row=4, n_col=7, images=Greenlight_images)

Yellowlight_images = list()
for i in range(34):
    file = "./yellowlight/" + "yel{0:02d}.png".format(i+1)
    img = cv2.imread(file)
    if img is None:
        print("파일이 없습니다")
        break
...
plot_images(n_row=2, n_col=17, images=Yellowlight_images)
...
for i in range(10):
    file = "./ex/" + "ex{0:02d}.png".format(i+1)
    img = cv2.imread(file)
    if img is None:
        print("파일이 없습니다")
        break
